...
% Version 2 - evaluates errors on all training data for RBF & ARD kernel 

disp('----- Active Learning Multi-Seed Experiment -----') 
disp('Preparing Data...') 

% Declare variables 
inf_func = @infExactW_mod; 
kernels = {@call_pgp_rbf_v2, @call_pgp_ard_v2};
kern_id = ['rbf'; 'ard']; 
...
for f = 1:2 
    ...
    for i=1:numel(tr_set)
        tr_ind_2{i}=find(data(:,1)==rid(tr_set(i)));
    end
    
    % For each kernel: 
    for kern = 1:numel(kernels) 
        
        disp(['----- Kernel ' kern_id(kern, 1:end) ' -----']) 
...
%        for s = 5:9 
            ...
            % MODEL 1: sGP 
            % Call pgp function 
            [so, tst, ~] = kernels{kern}(X, Y, Yind, tr_ind, tr_ind_2, tst_ind, inf_func, [], false, false); 
            
            % Evaluate error for each patient 
            all_tst_error = [];
            for pat = 1:numel(tr_ind_2)
            ...
            m1_errors = [rid(tr_set) all_tst_error]; 
            
            % Save errors 
            csvwrite([currentDir '/al_exp_seed_results_v2/f' num2str(f) '_s' num2str(s) '_m1_error_' kern_id(kern, :) '.csv'], m1_errors);
            ...
            while length(tr_ind_not_sel{1}) > 50
                ...
                % Call pgp function 
                [so, tst, add_inds] = kernels{kern}(X, Y, Yind, tr_ind_sel, tr_ind_2, tst_ind, inf_func, tr_ind_not_sel, true, false); 
            ...
            % Save errors 
            csvwrite([currentDir '/al_exp_seed_results_v2/f' num2str(f) '_s' num2str(s) '_m2_error_' kern_id(kern, :) '.csv'], m2_iter_errors);
            ...
            while length(tr_ind_not_sel{1}) > 50
                ...
                % Call pgp function 
                [so, tst, add_inds] = kernels{kern}(X, Y, Yind, tr_ind_sel, tr_ind_2, tst_ind, inf_func, tr_ind_not_sel, false, true); 
            ...
            % Save errors 
            csvwrite([currentDir '/al_exp_seed_results_v2/f' num2str(f) '_s' num2str(s) '_m3_error_' kern_id(kern, :) '.csv'], m3_iter_errors);            
            ...
            while length(tr_ind_not_sel{1}) > 50
                ...
                % Call pgp function 
                [so, tst, ~] = kernels{kern}(X, Y, Yind, tr_ind_sel, tr_ind_2, tst_ind, inf_func, [], false, false); 
            ...
            % Save errors 
            csvwrite([currentDir '/al_exp_seed_results_v2/f' num2str(f) '_s' num2str(s) '_m4_error_' kern_id(kern, :) '.csv'], m4_iter_errors);
