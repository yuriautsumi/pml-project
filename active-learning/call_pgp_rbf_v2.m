function [so, tst, add_inds] = call_pgp_rbf_v2(X, Y, Yind, tr_ind, tr_ind_2, tst_ind, inf_func, tr_ind_not_sel, error_bool, uncertainty_bool)
...
% Version 2 - evaluates error on training data 
...
for j = 1:numel(tr_ind_2)
    
    % extract test data
    xtest  = X(tr_ind_2{j},:);
    ytstar = Y(tr_ind_2{j},:);
    ytind = Yind(tr_ind_2{j},:);
