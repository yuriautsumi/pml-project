function [so, tst, add_inds] = call_pgp_rbf_threshold(X, Y, Yind, tr_ind, tst_ind, inf_func, tr_ind_not_sel, error_bool, uncertainty_bool, error_val, uncertainty_val)
% Input: X, Y, indicators, training and testing indices, inference function, not
% selected indices (optional), error sampling boolean, uncertainty
% sampling, error threshold value, uncertainty threshold value 
...
if error_bool 
    ...
    max_error_inds = find(diff > error_val); % get indices over error threshold 
    add_inds = tr_ind_not_sel{:}(max_error_inds); % get max error training indices 
...
elseif uncertainty_bool 
    ...
    % Get add_inds 
    max_var_inds = find(not_sel.s_s > uncertainty_val); % get indices over variance threshold 
    add_inds = tr_ind_not_sel{:}(max_var_inds); % get max variance training indices 
