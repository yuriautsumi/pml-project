# Processing AL Experiment Results 
...
RESULTS_DIR = os.path.join(CURRENT_DIR, 'al_exp_results') 
...
x_ticks = [100] + list(range(150, 1900, 50)) + [1890]

rbf_graph_data = []
# For RBF: 
for f in folds:
    # Load CSV data for each model 
    m1_data = np.genfromtxt(os.path.join(RESULTS_DIR, 'f%s_m1_error_rbf.csv'%(f)), delimiter=',')
    m2_data = np.genfromtxt(os.path.join(RESULTS_DIR, 'f%s_m2_error_rbf.csv'%(f)), delimiter=',')
    m3_data = np.genfromtxt(os.path.join(RESULTS_DIR, 'f%s_m3_error_rbf.csv'%(f)), delimiter=',')
    ...
    graph_data = np.hstack((m1_processed, m2_data, m3_data))
...
plt.plot(x_ticks, rbf_graph_data_avg[:, 2:], linewidth=2.0, color='gray', label='Random Sampling')
plt.legend(loc=1)
...
for f in folds:
    # Load CSV data for each model 
    m1_data = np.genfromtxt(os.path.join(RESULTS_DIR, 'f%s_m1_error_ard.csv'%(f)), delimiter=',')
    m2_data = np.genfromtxt(os.path.join(RESULTS_DIR, 'f%s_m2_error_ard.csv'%(f)), delimiter=',')
    m3_data = np.genfromtxt(os.path.join(RESULTS_DIR, 'f%s_m3_error_ard.csv'%(f)), delimiter=',')
...
plt.plot(x_ticks, ard_graph_data_avg[:, 2:], linewidth=2.0, color='gray', label='Random Sampling')
plt.legend(loc=1)
plt.xlabel('Number of Training Data')
plt.ylabel('MSE')
plt.grid(True)
